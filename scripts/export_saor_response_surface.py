#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""安息角試験の応答曲面用データを出力する(手順書2.2.7)。

9条件すべての解析が完了した後に1回だけ実行する。

実行例:
    python scripts/export_saor_response_surface.py
    python scripts/export_saor_response_surface.py --workbook projects/0/saor_test/saor_analysis_test.xlsx
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="解析Excel(既定はsaor_test/saor_analysis.xlsx)",
    )
    parser.add_argument(
        "--output-csv", type=Path, default=None,
        help="出力先CSV(既定はsaor_test/saor_response_surface_data.csv)",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    workbook_path = arguments.workbook or layout.saor_workbook
    output_csv_path = arguments.output_csv or (
        workbook_path.parent / layout.saor_response_surface_csv.name
    )
