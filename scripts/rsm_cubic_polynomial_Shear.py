# -*- coding: utf-8 -*-
"""せん断試験の3次多項式応答曲面(RSM)を作成する。

実行例:
    python scripts/rsm_cubic_polynomial_Shear.py
    python scripts/rsm_cubic_polynomial_Shear.py --data-file <CSV> --output-dir <出力先>
"""
...
OUT_PREFIX = RSM_OUTPUT_PREFIXES[RSM_SHEAR_NAME] + 'cubic_poly_'
OUTPUT_DIR = Path('.')
GRID_N = 401
DPI = 300
...
def parse_arguments():
    ...
    parser.add_argument('--data-file', type=Path, default=None,
                        help='入力CSV(既定はshear_test/shear_response_surface_data.csv)')
    parser.add_argument('--output-dir', type=Path, default=None,
                        help='出力先フォルダ(既定はrsm/phase1/shear/outputs)')
...
def main():
    ...
    data_file=args.data_file or layout.shear_response_surface_csv
    ...
    if BASE_ENABLE:
        ...
        plt.figure(figsize=(8,6)); colors=plt.cm.tab10(np.linspace(0,1,len(x1_levels)))
        ...
        plt.figure(figsize=(8,6)); colors2=plt.cm.tab20(np.linspace(0,1,len(x2_levels)))
